...
later index by a constant, and the fit can only absorb that by trading slope
...
def board_timestamps(frame_times, offset=1234.5, slope=1.0, exposure=9.0, stride=1):
    """Board readings for a set of frames, exactly on the given affine map."""
    return {
        k: (slope * pts + offset, slope * pts + offset + exposure)
...
def test_fit_recovers_clock_on_a_clean_recording():
    ...
    assert fit.intercept == pytest.approx(1234.5, abs=1e-6)
...
def test_fit_ignores_a_dropout_instead_of_absorbing_it():
    ...
    # The container clock needs no rescaling, so the slope is 1 and the offset
    # is the true one -- not the 0.858x / -5.593 s the index fit produced.
    assert fit.slope == pytest.approx(1.0, abs=1e-4)
    assert fit.intercept == pytest.approx(1234.5, abs=1.0)
...
def test_fit_measures_a_drifting_clock():
    ...
    fit = fit_timeline(
        frame_times, board_timestamps(frame_times, slope=1.0000086, stride=30)
    )
    assert fit.slope == pytest.approx(1.0000086, abs=1e-9)
...
def test_affine_accepts_both_field_names():
    assert affine_from_statistics({"speed_factor": 1.5, "intercept": 2.0}) == (1.5, 2.0)
    # Device recordings name the same quantity "slope"
    assert affine_from_statistics({"slope": 1.5, "intercept": 2.0}) == (1.5, 2.0)


def test_affine_rejects_data_from_before_the_pts_fit():
    with pytest.raises(KeyError, match="Re-run rocsync"):
        affine_from_statistics({"speed_factor": 0.858, "first_frame": -5593.0})
